...
local function inherit(inheritName, converters)
    return {
        createPingChannel = function (name, registry, valueConverters, dispatching)
            local ret = {}
        ...
        inheritSettings = function(name, c)
            return inherit(inheritName .. "$" .. name, setmetatable(c, {__index = converters}))
        end
    }
end
